...
const Withdraw = () => {
  ...
  function validate(field: any){
    ...
    if(!field === (/^\d+$/.test(field))){
      setStatus('Error, input must contain only numbers');
      setTimeout(() => setStatus(''),2000);
      setTimeout(() => setBalance(Number(ctx.users[0].balance)),2000);
    }
    if(field > balance){
      setStatus('Sorry, you can\'t overdraft your account.');
      setTimeout(() => setStatus(''),2000);
      setTimeout(() => setBalance(Number(ctx.users[0].balance)),2000);
    }
      return true;
...
function handleCreate(){
  ...
  <Card.Body>
    ...
    <p>{status}</p>
    <button type='submit' disabled={button} onClick={handleCreate}>Withdraw Funds</button>
  </Card.Body>
</Card>
  )
